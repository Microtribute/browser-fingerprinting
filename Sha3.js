 ...
 * @see https://www.movable-type.co.uk/scripts/html
 * 
 * References:
 * @see https://keccak.noekeon.org/Keccak-reference-3.0.pdf
 * @see https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.202.pdf
 * 
 * @copyright Chris Veness 2016-2020
 * @license MIT Licence
 */
(function () {
  /*
   ...
   *
   * @example
   * const hash = hash224('abc'); // 'e642824c3f8cf24ad09234ee7d3c766fc9a3a5168d0c94ad73b46fdf'
   */
  function hash224(message, options) {
    return keccak1600(1152, 448, message, options);
  }
   ...
   * @returns {string} Hash as hex-encoded string.
   */
  function hash256(message, options) {
    return keccak1600(1088, 512, message, options);
  }
   ...
   * @returns {string} Hash as hex-encoded string.
   */
  function hash384(message, options) {
    return keccak1600(832, 768, message, options);
  }
   ...
   * @returns {string} Hash as hex-encoded string.
   */
  function hash512(message, options) {
    return keccak1600(576, 1024, message, options);
  }
   ...
   * @private
   */
  function keccak1600(r, c, M, options) {
    const defaults = { padding: 'sha-3', msgFormat: 'string', outFormat: 'bytes' };
    ...
    const bytes = transpose(state)
      .map(plane => plane.map(lane => lane.toString(16).padStart(16, '0').match(/.{2}/g).reverse()))
      .flat().flat().map(b => Number.parseInt(b, 16)).slice(0, l / 8);

    // if required, group message digest into bytes or words
    if ('hex' === opt.outFormat) {
      return bytes.map(b => (b.toString(16).padStart(2, '0'))).join('');
    } else if ('raw' === opt.outFormat) {
  ...
  window.Sha3 = { hash224, hash256, hash384, hash512 };
})();
